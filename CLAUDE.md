...
Tally is a single-page, fully client-side bill splitter (no backend, no bundler, no framework CLI). `index.html` loads every dependency from a CDN and boots one Vue 3 app defined in `js/app.js`. Live at https://getsuzym.github.io/Tally/.

There was a receipt-photo-scanning feature (OCR → auto-filled dishes) that went through two implementations — Tesseract.js (too inaccurate on real phone photos, verified directly) then Google Cloud Vision (accurate, but the per-user API key setup was too much friction) — before being removed. See git history if reviving it.
